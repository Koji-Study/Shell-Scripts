...
if type docker > /dev/null 2>&1 ; then
   ...
   && wget -O /etc/yum.repos.d/docker-ce.repo https://download.docker.com/linux/centos/docker-ce.repo  > /dev/null 2>&1 \
   && sed -i 's+download.docker.com+mirrors.tuna.tsinghua.edu.cn/docker-ce+' /etc/yum.repos.d/docker-ce.repo
   echo "Step3: 安装docker-ce"
   yum install -y docker-ce > /dev/null 2>&1
   echo "Step4: 启动并设置docker开机自启动"
   systemctl start docker \
   && systemctl enable docker
   echo "Step5: 设置docker的存储路径"
